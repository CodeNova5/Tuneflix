...
export default function HomePage() {
  ...
  return (
    <div className="p-4 bg-gray-900 text-white min-h-screen">
      ...
      <h1 className="text-3xl font-bold mb-4">Billboard Top Artists</h1>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {artists.map((artist, idx) => (
          <Link
            key={idx}
            href={`/Music/${encodeURIComponent(artist.artist)}`}
          >
            <div className="border rounded-lg p-2 shadow-md bg-gray-800 cursor-pointer">
              <img
                ...
                className="w-full h-48 object-cover rounded"
              />
              <h2 className="font-semibold mt-2">{artist.artist}</h2>
              <p className="text-gray-400">Rank: {artist.rank}</p>
            </div>
          </Link>
        ))}
      </div>
